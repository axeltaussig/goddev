...
class BuildResponse(BaseModel):
    ...
    message: str
    started_at: str
    cost_estimate: Optional[dict] = None  # {"estimated_cost_usd": float, "warning": str, "tips": []}
    class Config:
        extra = "allow"
...
async def _run_build(job_id: str, request: str, max_critic_iterations: int, max_iterations: int) -> None:
    ...
    try:
        ...
        # CRITICAL: Flush execution trace to disk for reflector/self-improver analysis
        from ..logger import trace as telemetry
        project_id = (final_state or {}).get("project_name", job_id)
        status = "SUCCESS" if (final_state or {}).get("integration_passed") else "COMPLETED"
        try:
            telemetry.close_run(project_id, status, n_nodes=0)
        except Exception as trace_error:
            print(f"[WARNING] Failed to flush telemetry: {trace_error}")
...
async def _run_self_build(job_id: str) -> None:
    ...
    try:
        ...
        # CRITICAL: Flush execution trace to disk so meta_cto can see what happened
        from ..logger import trace as telemetry
        project_id = (final_state or {}).get("project_name", job_id)
        status = "SUCCESS" if (final_state or {}).get("integration_passed") else "COMPLETED"
        try:
...
async def submit_build(body: BuildRequest):
    ...
    # Estimate cost for this build
    from .cost_tracker import estimate_build_cost
    cost_estimate = estimate_build_cost(
        body.request,
        max_iterations=body.max_iterations,
        max_critic_iterations=body.max_critic_iterations,
    )
...
@app.get("/build/{job_id}/cost-optimization")
async def get_cost_optimization(job_id: str):
    """Get real-time cost optimization tips for a running build."""
    job = _jobs.get(job_id)
    if not job:
        raise HTTPException(status_code=404, detail=f"Job {job_id} not found")
    ...
    cost_summary = job.get("cost_summary")
    if not cost_summary:
        return {"tips": [], "status": "no_data_yet"}
    
    tips = []
    total_cost = cost_summary.get("total_cost_usd", 0)
    total_tokens = cost_summary.get("total_tokens", 0)
    cost_per_million_tokens = cost_summary.get("cost_per_million_tokens", 0)
    
    # Analyze cost efficiency
    if total_cost > 50:
        tips.append({
            "severity": "high",
            "message": f"🚨 Build is expensive: ${total_cost:.2f} spent so far",
            "suggestion": "Consider stopping and simplifying your request (reduce iteration limits, split into smaller builds)"
        })
    elif total_cost > 20:
        tips.append({
            "severity": "medium",
            "message": f"💰 Build is moderate cost: ${total_cost:.2f}",
            "suggestion": "Monitor closely — you're spending more than average"
        })
    
    # Analyze by-agent costs
    by_agent = cost_summary.get("by_agent", {})
    expensive_agents = [(role, info) for role, info in by_agent.items() if info.get("cost_usd", 0) > 5]
    if expensive_agents:
        for role, info in expensive_agents:
            model = info.get("model", "unknown")
            cost = info.get("cost_usd", 0)
            tips.append({
                "severity": "info",
                "message": f"{role} using {model}: ${cost:.2f}",
                "suggestion": f"This agent is expensive — {model} charges high rates"
            })
    
    # Check cost per token efficiency
    if cost_per_million_tokens > 5:  # More than $5 per 1M tokens
        tips.append({
            "severity": "warning",
            "message": f"⚠️ Cost per token is high: ${cost_per_million_tokens:.2f}/M tokens",
            "suggestion": "The router is choosing expensive models. This might indicate high complexity or fallback chains being triggered."
        })
...
async def websocket_stream(websocket: WebSocket, job_id: str):
    """Stream real-time agent output for a build job."""
    ...
    # Send any events that already happened (catch-up)
    for event in _job_events.get(job_id, []):
        try:
            await websocket.send_text(json.dumps({
                "type": _classify_event(event), "content": event
            }))
        except Exception:
            break
